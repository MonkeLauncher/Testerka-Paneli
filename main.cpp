...
CircleShape button (71, 6), numberButton (71, 6);
RectangleShape textWindow, dark;
Font minecrafter;
Text buttonText [2], fileText, errorText [2], speedTitle, frameTitle, numberButtonText, numbers [20];

string saveName, tempSaveName;
int frame = 0, framesAmount = 0, settingFrame = 0;
double frameTime = 1;
bool changeFrame = true, newFile = false, fileError = false, showNumbers = false;
...
void preprocessing () {
	...
	numberButton.setFillColor (Color (10, 10, 10));
	numberButton.setPosition (3 * 71, 3 * 125 + 30);
	...
	numberButtonText.setFillColor (Color (230, 230, 230));
	numberButtonText.setCharacterSize (20);
	numberButtonText.setFont (minecrafter);
	numberButtonText.setString ("Numeracja");
	numberButtonText.setPosition (numberButton.getPosition().x + (numberButton.getRadius() - (numberButtonText.getLocalBounds().width / 2)), 
								  numberButton.getPosition().y + 58);
								  
	for (int i = 0; i < 20; i++) {
		numbers[i].setOutlineColor (Color (10, 10, 10));
		numbers[i].setOutlineThickness (3);
		numbers[i].setFillColor (Color (230, 230, 230));
		numbers[i].setCharacterSize (40);
		numbers[i].setFont (minecrafter);
		numbers[i].setString (to_string (i + 1));
		numbers[i].setPosition (hexagon[i].getPosition().x + (hexagon[i].getRadius() - (numbers[i].getLocalBounds().width / 2)), hexagon[i].getPosition().y + 47);
	}
...
void drawAll () {
	...
	okno.draw (button);
	okno.draw (numberButton);
	for (int i = 0; i < 2; i++) okno.draw (buttonText[i]);
	okno.draw (numberButtonText);
	
	if (showNumbers) {
		for (int i = 0; i < 20; i++) okno.draw (numbers[i]);
	}
...
void reacting () {
	...
	if (newFile) {
		...
		if (numberButton.getGlobalBounds().contains (MPos(okno).x, MPos(okno).y)) {
			numberButton.setFillColor (Color (40, 40, 40));
			
			if (MBumped (Mouse::Left)) {
				numberButton.setFillColor (Color (0, 0, 0));
				showNumbers = !(showNumbers);
			}
		}
		else numberButton.setFillColor (Color (0, 0, 0));
